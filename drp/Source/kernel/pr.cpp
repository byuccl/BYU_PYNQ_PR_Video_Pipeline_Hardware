...
static unsigned char lower_limit = 0;
static unsigned char upper_limit = 255;
static char kernel [9] = {0,0,0,0,1,0,0,0,0};
...
pixel_type sobel_operator(WINDOW *window)
{
	...
	char x_op[3][3] = { {kernel[0],kernel[1],kernel[2]},
			{kernel[3],kernel[4],kernel[5]},
			{kernel[6],kernel[7],kernel[8]}};

	char y_op[3][3] = { {kernel[0],kernel[3],kernel[6]},
	...
	for(i=0; i < 3; i++){
		for(j = 0; j < 3; j++){
			printf("Window %d X_OP: %d\t",window->getval(i,j),x_op[i][j]);
	...
	edge_val = ((unsigned char)(edge_val_shifted));
	printf("Edge Value : %d\n",edge_val);
